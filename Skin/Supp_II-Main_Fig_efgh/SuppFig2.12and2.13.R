  ...
  for (j in 1:bootstrap_no){
    ...
    for (k in 1:length(types)){
      ...
      if (length(which(rowSums(sign(otu_table(rZOTUtemp)))>=core*sample_size))>1){
        ...
        Faithcount<-c(Faithcount,0)
        Faithcountall<-c(Faithcountall,0)
        Faithpercent<-c(Faithpercent, 0)}
...
ggsave("Figure_SI2.12.png", grid_rich, height = 20, width = 20)
...
ggsave("Figure_SI2.13.png", grid_faith, height = 20, width = 20)
