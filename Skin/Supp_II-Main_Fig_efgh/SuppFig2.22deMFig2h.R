...
types<-c("SBluG_ino" , "SNW_marm" , "SBluG_neo" , "SNW_neo")#unique(type_full)
...
p_richness<-p_richness+annotate("text", size=6,x=2.5, y=15, label= "Kruskal-Wallis")
p_richness<-p_richness+annotate("text", size=6,x=2.5, y=11, label= paste("p = ",round(kw_richness$p.value,4)))
...
#Add the p-value for the Kruskal-Wallis test somewhere on your figure (you may have to change the x and y positions of this label)
p_faith<-p_faith+annotate("text", size=6,x=2.5, y=20, label= "Kruskal-Wallis")
p_faith<-p_faith+annotate("text", size=6,x=2.5, y=15, label= paste("p = ",round(kw_faith$p.value,4)))
...
#Make your plot
p_faith #SI Fig. 2.22e & Main Fig. 2h
