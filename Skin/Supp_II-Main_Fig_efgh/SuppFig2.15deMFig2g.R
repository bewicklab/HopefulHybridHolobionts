...
for (j in 1:bootstrap_no){
  ...
  in1and2<-na.omit(setdiff(setdiff(intersect(corenames_sort[[1]],corenames_sort[[2]]),corenames_sort[[3]]),corenames_sort[[4]]))
...
p_richness<-p_richness+annotate("text", size=6,x=2.5, y=100, label= "Kruskal-Wallis")
p_richness<-p_richness+annotate("text", size=6,x=2.5, y=65, label= paste("p = ",round(kw_richness$p.value,4)))
...
p_faith<-p_faith+annotate("text", size=6,x=2.5, y=13, label= paste("p = ",round(kw_faith$p.value,4)))
...
#Make your plot
p_faith #SI Fig. 2.15e & Main Fig. 2g
