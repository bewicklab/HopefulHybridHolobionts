...
#Taxa that are independently part of both progenitor cores, but not on neos
...
#Check to see if taxa that are part of the ino core, are on marms (though not part of core), but not on neos
print('Core ino not on hybrid:')
core_ino_not_on_neo_on_marm<-setdiff(intersect(core_ino_names,in_marm_names),in_neo_names)
for (k in 1:length(core_ino_not_on_neo_on_marm)){
  print(tax_table(inoZOTU)[which(rownames(tax_table(inoZOTU))==core_ino_not_on_neo_on_marm[k])])
}
...
#Taxa that are part of the neo core, but not on either marms or inos
print('Core hybrid not on marms:')
core_neo_not_on_progenitors<-setdiff(core_neo_names,union(in_marm_names,in_ino_names))
for (k in 1:length(core_neo_not_on_progenitors)){
  print(tax_table(neoZOTU)[which(rownames(tax_table(neoZOTU))==core_neo_not_on_progenitors[k])])
}#Nothing
