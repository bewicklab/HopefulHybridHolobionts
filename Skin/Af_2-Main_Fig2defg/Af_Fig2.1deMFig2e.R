...
tree_file<-read.tree('rooted_tree_out/tree.nwk')
...
p_richness<-p_richness+annotate("text", size=6,x=2, y=790, label= "Kruskal-Wallis")
p_richness<-p_richness+annotate("text", size=6,x=2, y=725, label= paste("p = ",round(kw_richness$p.value,4)))
...
#Add the p-value for the Kruskal-Wallis test somewhere on your figure (you may have to change the x and y positions of this label)
p_faith<-p_faith+annotate("text", size=6,x=2, y=67, label= "Kruskal-Wallis")
p_faith<-p_faith+annotate("text", size=6,x=2, y=63, label= paste("p = ",round(kw_faith$p.value,4)))
...
#Make your plot
p_faith #SI Fig. 2.1e & Main Fig. 2e
