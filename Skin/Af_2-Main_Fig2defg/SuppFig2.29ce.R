...
#Change line 68 to line 70 to rarefy to the read depth of gut samples (Supplemental Figure 21e)
#rZOTU<-rarefy_even_depth(ZNC_physeq,rngseed = 1, sample.size=11412)
...
checker33<-(otu_table(rZOTU)[which(rownames(otu_table(rZOTU)) %in% in3),which(sample_data(rZOTU)$speciesxsite %in% c(types[1],types[2],types[4]))])
...
seqs33<-which(rowSums(checker33)==0)
seqs4<-which(rowSums(checker4)==0)
seqs34<-which(rowSums(checker34)==0)
assigned_taxa[which(assigned_taxa[,1] %in% rownames(checker3[seqs3])),2]
...
write.csv(sharing,'ASV_skin_Venn.csv')
