...
for (kk in 1:sample_size){
  ...
  for (j in 1:bootstrap_no){
    ...
    boot_tracker<-rbind(boot_tracker,corecount)
    faith_boot_tracker<-rbind(faith_boot_tracker,Faithcount)
  ...
  if (kw_faith$p.value<0.05 && abs(max(diversity_df$faith)-min(diversity_df$faith))>1E-10){
...
if (kw_faith$p.value<0.05 && abs(max(diversity_df$faith)-min(diversity_df$faith))>1E-10){
  ...
  #  new_y<-3.75
  #  y_step<-0.3
  new_y<-1.1*max(diversity_df[,3])
  y_step<-0.1*(max(diversity_df[,3])-min(diversity_df[,3]))
  #For each pairwise comparison...
...
rich_line_core <- ggplot() + theme_classic(16) + ylab("Core Richness (Skin ASV)") + xlab("Minimum Hosts") +
...
rich_log_line_core<-ggplot() + theme_classic(16) + ylab("Log Core Richness (Skin ASV)") + xlab("Minimum Hosts") +
  scale_x_continuous(breaks = seq(0, 12, by = 2)) + expand_limits(y=2.5) + #setting x-axis interval and y-axis minimum
...
ggsave("Figure_SI2.5.png", grid_rich, height = 20, width = 20)
...
kaxis<-(1:1:sample_size)
faith_line_core <- ggplot() + theme_classic(16) + ylab("Core Faith's PD (Skin ASV)") + xlab("Minimum Hosts") +
  scale_x_continuous(breaks = seq(0, 12, by = 2)) + expand_limits(y=0) + #setting x-axis interval and y-axis minimum
...
faith_log_line_core<-ggplot() + theme_classic(16) + ylab("Log Core Faith's PD (Skin ASV)") + xlab("Minimum Hosts") +
  scale_x_continuous(breaks = seq(0, 12, by = 2)) + expand_limits(y=10.) + #setting x-axis interval and y-axis minimum
...
ggsave("Figure_SI2.6.png", grid_faith, height = 20, width = 20)
