...
for (kk in 1:sample_size){
  ...
  for (j in 1:bootstrap_no){
    ...
    all1<-na.omit(corenames_sort[[1]])
    all2<-na.omit(corenames_sort[[2]])
    all3<-na.omit(corenames_sort[[3]])
    all4<-na.omit(corenames_sort[[4]])
    
    
    #Percentage of taxa unique to SBluG_neo
    uniques<-c(100*length(in1)/length(all1),100*length(in2)/length(all2),100*length(in3)/length(all3),100*length(in4)/length(all4))

    faithsuniques<-c()
    if (length(in1)>0){
    rZOTUin1 = prune_taxa(in1, rZOTU)
    rZOTUall1 = prune_taxa(all1, rZOTU)
    if (length(which(rowSums(sign(otu_table(rZOTUin1)))>=core*sample_size))>1){
      onematrixin1<-sign(t(otu_table(rZOTUin1))+1)
      onematrixall1<-sign(t(otu_table(rZOTUall1))+1)
      faithpdin1<-pd(onematrixin1, phy_tree(rZOTUin1), include.root=FALSE)
      faithpdall1<-pd(onematrixall1, phy_tree(rZOTUall1), include.root=FALSE)
      faithsin1<-faithpdin1[,1]
      faithsall1<-faithpdall1[,1]
      faithsuniques<-c(faithsuniques,100*faithsin1[1]/faithsall1[1])}
    else{
  ...
  if (length(in2)>0){ 
    ...
    rZOTUall2 = prune_taxa(all2, rZOTU)
    if (length(which(rowSums(sign(otu_table(rZOTUin2)))>=core*sample_size))>1){
      onematrixin2<-sign(t(otu_table(rZOTUin2))+1)
      onematrixall2<-sign(t(otu_table(rZOTUall2))+1)
      faithpdin2<-pd(onematrixin2, phy_tree(rZOTUin2), include.root=FALSE)
      faithpdall2<-pd(onematrixall2, phy_tree(rZOTUall2), include.root=FALSE)
      faithsin2<-faithpdin2[,1]
      faithsall2<-faithpdall2[,1]
      faithsuniques<-c(faithsuniques,100*faithsin2[1]/faithsall2[1])}
    else{
      faithsuniques<-c(faithsuniques,0)
    }
  }
  ...
  if (length(in3)>0){  
    ...
    rZOTUall3 = prune_taxa(all3, rZOTU)
    if (length(which(rowSums(sign(otu_table(rZOTUin3)))>=core*sample_size))>1){
      onematrixin3<-sign(t(otu_table(rZOTUin3))+1)
      onematrixall3<-sign(t(otu_table(rZOTUall3))+1)
      faithpdin3<-pd(onematrixin3, phy_tree(rZOTUin3), include.root=FALSE)
      faithpdall3<-pd(onematrixall3, phy_tree(rZOTUall3), include.root=FALSE)
      faithsin3<-faithpdin3[,1]
      faithsall3<-faithpdall3[,1]
      faithsuniques<-c(faithsuniques,100*faithsin3[1]/faithsall3[1])}
    else{
      faithsuniques<-c(faithsuniques,0)
    }
  }
  ...
  if (length(in4)>0){
    ...
    rZOTUall4 = prune_taxa(all4, rZOTU)
    if (length(which(rowSums(sign(otu_table(rZOTUin4)))>=core*sample_size))>1){
      onematrixin4<-sign(t(otu_table(rZOTUin4))+1)
      onematrixall4<-sign(t(otu_table(rZOTUall4))+1)
      faithpdin4<-pd(onematrixin4, phy_tree(rZOTUin4), include.root=FALSE)
      faithpdall4<-pd(onematrixall4, phy_tree(rZOTUall4), include.root=FALSE)
      faithsin4<-faithpdin4[,1]
      faithsall4<-faithpdall4[,1]
      faithsuniques<-c(faithsuniques,100*faithsin4[1]/faithsall4[1])}
    else{
      faithsuniques<-c(faithsuniques,0)
    }
  }
  ...
  colnames(faith_boot_tracker)<-c("SBluG_ino" , "SNW_marm" , "SBluG_neo" , "SNW_neo")
  ...
  faith_SBluG_ino<-c(faith_SBluG_ino,median(faith_boot_tracker[,1]))
  faith_SBluG_neo<-c(faith_SBluG_neo,median(faith_boot_tracker[,3]))
  faith_SNW_neo<-c(faith_SNW_neo,median(faith_boot_tracker[,4]))
  faith_SNW_marm<-c(faith_SNW_marm,median(faith_boot_tracker[,2]))
  ...
  #Richness
  if (length(na.omit(diversity_df$richness))>0){
  kw_richness<-kruskal.test(richness ~ type, data = diversity_df)
  if (is.na(kw_richness$p.value)){kw_richness$p.value<-1}
  if (kw_richness$p.value<0.05 && abs(max(na.omit(diversity_df$richness))-min(na.omit(diversity_df$richness)))>1E-10){
    print('Core Richness')
  ...
  if (length(na.omit(diversity_df$faith))>0){
  kw_faith<-kruskal.test(faith ~ type, data = diversity_df)
  if (is.na(kw_faith$p.value)){kw_faith$p.value<-1}
  if (kw_faith$p.value<0.05 && abs(max(na.omit(diversity_df$faith))-min(na.omit(diversity_df$faith)))>1E-10){
  ...
  if (kw_richness$p.value<0.05 && abs(max(na.omit(diversity_df$richness))-min(na.omit(diversity_df$richness)))>1E-10 && length(na.omit(diversity_df$richness))>0){
    ...
    #  new_y<-22
    #  y_step<-2
    new_y<-1.1*max(na.omit(diversity_df[,2]))
    y_step<-0.1*(max(na.omit(diversity_df[,2]))-min(na.omit(diversity_df[,2])))
...
if (kw_faith$p.value<0.05 && abs(max(na.omit(diversity_df$faith))-min(na.omit(diversity_df$faith)))>1E-10 && length(na.omit(diversity_df$faith))>0){
  #Names of the groups you're comparing
  group1<-c()
  group2<-c()
  #p-value for the pairwise comparisons
  ...
  #  new_y<-3.75
  #  y_step<-0.3
  new_y<-1.1*max(na.omit(diversity_df[,3]))
  y_step<-0.1*(max(na.omit(diversity_df[,3]))-min(na.omit(diversity_df[,3])))
  #For each pairwise comparison...
...
p_faith <- NULL
mylist_faith[[kk]] <- local({
kk <- kk   

#Define violin plot
p_faith <- ggplot(diversity_df, aes(x=type, y=faith)) + geom_violin(aes(fill=type)) + theme(axis.title.x = element_blank())
#Choose the size of font for the axes titles and labels
p_faith<-p_faith+theme(axis.title = element_text(size = 20))+theme(axis.text.y = element_text(size = 15))+theme(axis.text.x = element_text(size = 12))
#Choose the size of font for the legend title and lables
p_faith<-p_faith+theme(legend.position="none")
...
#Add the p-value for the Kruskal-Wallis test somewhere on your figure (you may have to change the x and y positions of this label)
p_faith<-p_faith+annotate("text", -Inf, Inf, hjust = 0, vjust = 1, size=6, label= paste('faiths pd:',paste0(round(core*100,2),'%'))) #x=2.9, y=0.9*min(diversity_df[,3]),
#p_faith<-p_faith+annotate("text", size=6,x=2.9, y=1, label= paste("p = ",round(kw_faith$p.value,4)))
...
#save richness plots to list
myplots_faith <- append(mylist_faith, p_faith)
...
rich_line_core <- ggplot() + theme_classic(16) + ylab("Unique Core Richness (Gut ASV)") + xlab("Minimum Hosts") +
...
rich_log_line_core<-ggplot() + theme_classic(16) + ylab("Log Unique Core Richness (Gut ASV)") + xlab("Minimum Hosts") +
  scale_x_continuous(breaks = seq(0, 12, by = 2)) + expand_limits(y=0) + #setting x-axis interval and y-axis minimum
...
ggsave("Figure_SI2.23.png", grid_rich, height = 20, width = 20)



############### Faith's PD of the core microbiome ####################


kaxis<-(1:1:sample_size)
faith_line_core <- ggplot() + theme_classic(16) + ylab("Unique Core Faith's PD (Gut ASV)") + xlab("Minimum Hosts") +
  scale_x_continuous(breaks = seq(0, 12, by = 2)) + expand_limits(y=0) + #setting x-axis interval and y-axis minimum
  geom_line(aes(x=kaxis, y=faith_SBluG_ino), colour='red') + geom_point(aes(x=kaxis, y=faith_SBluG_ino), size=3.5, colour='red') +
  geom_line(aes(x=kaxis, y=faith_SBluG_neo), colour='magenta') + geom_point(aes(x=kaxis, y=faith_SBluG_neo), size=3.5, colour='magenta') +
  geom_line(aes(x=kaxis, y=faith_SNW_neo), colour='lightslateblue') + geom_point(aes(x=kaxis, y=faith_SNW_neo), size=3.5, colour='lightslateblue') +
  geom_line(aes(x=kaxis, y=faith_SNW_marm), colour='blue') + geom_point(aes(x=kaxis, y=faith_SNW_marm), size=3.5, colour='blue') +
  theme(axis.title = element_text(size = 25))+theme(axis.text = element_text(size = 20))

faith_log_line_core<-ggplot() + theme_classic(16) + ylab("Log Unique Core Faith's PD (Gut ASV)") + xlab("Minimum Hosts") +
  scale_x_continuous(breaks = seq(0, 12, by = 2)) + expand_limits(y=0) + #setting x-axis interval and y-axis minimum
  geom_line(aes(x=if (length(faith_SBluG_ino[log(faith_SBluG_ino)>=0])==12) (kaxis) else head(kaxis,-(12-length(faith_SBluG_ino[log(faith_SBluG_ino)>=0]))), y=log(faith_SBluG_ino[log(faith_SBluG_ino)>=0])), colour='red') + 
  geom_point(aes(x=if (length(faith_SBluG_ino[log(faith_SBluG_ino)>=0])==12) (kaxis) else head(kaxis,-(12-length(faith_SBluG_ino[log(faith_SBluG_ino)>=0]))), y=log(faith_SBluG_ino[log(faith_SBluG_ino)>=0])), size=3.5, colour='red') +
  geom_line(aes(x=if (length(faith_SBluG_neo[log(faith_SBluG_neo)>=0])==12) (kaxis) else head(kaxis,-(12-length(faith_SBluG_neo[log(faith_SBluG_neo)>=0]))), y=log(faith_SBluG_neo[log(faith_SBluG_neo)>=0])), colour='magenta') + 
  geom_point(aes(x=if (length(faith_SBluG_neo[log(faith_SBluG_neo)>=0])==12) (kaxis) else head(kaxis,-(12-length(faith_SBluG_neo[log(faith_SBluG_neo)>=0]))), y=log(faith_SBluG_neo[log(faith_SBluG_neo)>=0])), size=3.5, colour='magenta') +
  geom_line(aes(x=if (length(faith_SNW_neo[log(faith_SNW_neo)>=0])==12) (kaxis) else head(kaxis,-(12-length(faith_SNW_neo[log(faith_SNW_neo)>=0]))), y=log(faith_SNW_neo[log(faith_SNW_neo)>=0])), colour='lightslateblue') + 
  geom_point(aes(x=if (length(faith_SNW_neo[log(faith_SNW_neo)>=0])==12) (kaxis) else head(kaxis,-(12-length(faith_SNW_neo[log(faith_SNW_neo)>=0]))), y=log(faith_SNW_neo[log(faith_SNW_neo)>=0])), size=3.5, colour='lightslateblue') +
  geom_line(aes(x=if (length(faith_SNW_marm[log(faith_SNW_marm)>=0])==12) (kaxis) else head(kaxis,-(12-length(faith_SNW_marm[log(faith_SNW_marm)>=0]))), y=log(faith_SNW_marm[log(faith_SNW_marm)>=0])), colour='blue') + 
  geom_point(aes(x=if (length(faith_SNW_marm[log(faith_SNW_marm)>=0])==12) (kaxis) else head(kaxis,-(12-length(faith_SNW_marm[log(faith_SNW_marm)>=0]))), y=log(faith_SNW_marm[log(faith_SNW_marm)>=0])), size=3.5, colour='blue') +
  theme(axis.title = element_text(size = 25))+theme(axis.text = element_text(size = 20))


gs_faith <- list(faith_line_core, faith_log_line_core, myplots_faith[[1]], myplots_faith[[2]], myplots_faith[[3]], myplots_faith[[4]], 
                 myplots_faith[[5]], myplots_faith[[6]], myplots_faith[[7]], myplots_faith[[8]], 
                 myplots_faith[[9]], myplots_faith[[10]], myplots_faith[[11]], myplots_faith[[12]])

lay_faith <- rbind(c(1,1,2,2),
                   c(1,1,2,2),
                   c(1,1,2,2),
                   c(3,4,5,6),
                   c(7,8,9,10),
...
grid_faith <- grid.arrange(grobs = gs_faith, layout_matrix = lay_faith)

grid.draw(grid_faith) # interactive device

ggsave("Figure_SI2.24.png", grid_faith, height = 20, width = 20)
