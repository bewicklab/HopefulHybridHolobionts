...
urZOTU<-phyloseq(otu_table(rZOTU),tax_table(rZOTU),sample_data(rZOTU),ultrametric_tree)


#Calculate Rao's entropy on the ultrametric tree
raopd<-raoD(comm=t(otu_table(rZOTU)),phy=phy_tree(urZOTU))
raos<-raopd$Dkk
...
p_richness<-p_richness+annotate("text", size=6,x=1, y=500, label= "Kruskal-Wallis")
p_richness<-p_richness+annotate("text", size=6,x=1, y=460, label= paste("p = ",round(kw_richness$p.value,4)))
...
#Add the p-value for the Kruskal-Wallis test somewhere on your figure (you may have to change the x and y positions of this label)
p_shannon<-p_shannon+annotate("text", size=6,x=3, y=1.3, label= "Kruskal-Wallis")
p_shannon<-p_shannon+annotate("text", size=6,x=3, y=0.8, label= paste("p = ",round(kw_shannon$p.value,6)))
...
#Add the p-value for the Kruskal-Wallis test somewhere on your figure (you may have to change the x and y positions of this label)
p_simpson<-p_simpson+annotate("text", size=6,x=3, y=0.31, label= "Kruskal-Wallis")
p_simpson<-p_simpson+annotate("text", size=6,x=3, y=0.25, label= paste("p = ",round(kw_simpson$p.value,6)))
...
#Add the p-value for the Kruskal-Wallis test somewhere on your figure (you may have to change the x and y positions of this label)
#p_faiths<-p_faiths+annotate("text", size=6,x=1, y=44, label= "Kruskal-Wallis")
#p_faiths<-p_faiths+annotate("text", size=6,x=1, y=40, label= paste("p = ",round(kw_faiths$p.value,6)))
  ...
  for (k in 1:length(rownames(pw_faiths$p.value))){
    for (j in 1:k){
      if (rownames(pw_faiths$p.value)[k]!=colnames(pw_faiths$p.value)[j]){
        #If there is a significant difference or you want to also show non-significant differences...
        if (pw_faiths$p.value[k,j]<0.07 || p_value_ns=='yes'){
          #Add an entry to your tibble include the names of the two groups being compared and the p-value for the comparison
...
#Make your plot
p_faiths #SI Fig. 1.1d & Main Fig. 1a
...
p_raos<-p_raos+annotate("text", size=6,x=2, y=0.125, label= paste("p = ",round(kw_raos$p.value,6)))
