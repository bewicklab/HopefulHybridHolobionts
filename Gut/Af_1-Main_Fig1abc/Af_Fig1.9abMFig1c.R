...
p_richness<-p_richness+annotate("text", size=6,x=3, y=1500, label= "Kruskal-Wallis")
p_richness<-p_richness+annotate("text", size=6,x=3, y=1350, label= paste("p = ",round(kw_richness$p.value,4)))
...
#Add the p-value for the Kruskal-Wallis test somewhere on your figure (you may have to change the x and y positions of this label)
#p_faiths<-p_faiths+annotate("text", size=6,x=3, y=80, label= "Kruskal-Wallis")
#p_faiths<-p_faiths+annotate("text", size=6,x=3, y=75, label= paste("p = ",round(kw_faiths$p.value,4)))
...
#Make your plot
p_faiths #SI Fig. 1.9b & Main Fig. 1c
