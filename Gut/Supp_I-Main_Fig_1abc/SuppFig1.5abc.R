...
#Find the richness, shannon entropy, simpson's index, and faiths pd for each sample
...
p_richness<-p_richness+annotate("text", size=6,x=1, y=15, label= "Kruskal-Wallis")
p_richness<-p_richness+annotate("text", size=6,x=1, y=3, label= paste("p = ",round(kw_richness$p.value,4)))
...
#Add the p-value for the Kruskal-Wallis test somewhere on your figure (you may have to change the x and y positions of this label)
p_shannon<-p_shannon+annotate("text", size=6,x=3, y=0.5, label= "Kruskal-Wallis")
p_shannon<-p_shannon+annotate("text", size=6,x=3, y=0.2, label= paste("p = ",round(kw_shannon$p.value,6)))
    ...
    for (j in 1:k){
      if (rownames(pw_shannon$p.value)[k]!=colnames(pw_shannon$p.value)[j]){
        ...
        if (pw_shannon$p.value[k,j]<0.1 || p_value_ns=='yes'){
          ...
          p.adj<-round(c(p.adj,as.numeric(pw_shannon$p.value[k,j])),7)
          #Add the y-position of the bracket and bump the y-location of the bracket up one for next time
          ypos<-c(ypos,new_y)
          new_y<-new_y+y_step
        }
...
#Add the p-value for the Kruskal-Wallis test somewhere on your figure (you may have to change the x and y positions of this label)
p_simpson<-p_simpson+annotate("text", size=6,x=3, y=0.2, label= "Kruskal-Wallis")
p_simpson<-p_simpson+annotate("text", size=6,x=3, y=0.12, label= paste("p = ",round(kw_simpson$p.value,6)))
